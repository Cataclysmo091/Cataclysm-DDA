...
const tripoint p1( 0, 0, -2 );
const tripoint p2( ( MAPSIZE *SEEX ) - 1, ( MAPSIZE *SEEY ) - 1, 1 );
        ...
        for( unsigned int i = 0; i < spawn_areas.size(); ++i ) {
            ...
            if( spawn_area.contains( p ) ) {
                ...
                inclusive_cuboid<tripoint> interior{
                    { spawn_area.area.p_min.xy() + point_south_east, spawn_area.area.p_min.z },
                    { spawn_area.area.p_max.xy() - point_south_east, spawn_area.area.p_min.z } };
...
constexpr int building_width = 7;
constexpr int building_height = 1;
constexpr tripoint building_offset{ building_width, building_width, building_height };
constexpr tripoint cave_offset{ building_width, building_width, 0 };
